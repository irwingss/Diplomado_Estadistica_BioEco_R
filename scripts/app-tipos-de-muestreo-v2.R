    ...
    sidebarLayout(
      sidebarPanel(
        ...
        conditionalPanel(
          ...
          selectInput("stratify_by", "Estratificar por:",
                      choices = c("Filas", "Columnas"))
        ),
        conditionalPanel(
  ...
  server <- function(input, output) {
    ...
    output$grid_plot <- renderPlot({
      ...
      p <- ggplot(grid, aes(x = x, y = y)) +
        ...
        geom_text(aes(label = id), size = 8, color = "gray50")+
        theme_minimal() +
        coord_fixed() +
      ...
      if (method == "Muestreo Estratificado") {
        if (input$stratify_by == "Filas") {
          p <- p + labs(subtitle = "Estratificación por Filas")
        } else {
          p <- p + labs(subtitle = "Estratificación por Columnas")
        }
