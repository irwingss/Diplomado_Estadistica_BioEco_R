...
table(dt1) |> sort(decreasing = TRUE)

```

# 2. Medidas de dispersión: rango, varianza, desviación estándar y coeficiente de variación

Si ustedes van a mostrar un promedio, deben acompañarlo de su desviación estándar: promedio = 5.6 (+-1.2)

Si ustedes van a mostrar una mediana, deben acompañarla de su RANGO INTERCUANTIL (IQR): mediana = 3 (1-5)
...
boxplot.stats(dt6)$out
```

Comprobemos la aparición de outliers en la data

```{r}
# Originalmente teníamos como outlier al 16
dt6 <- c(5, 7, 3, 8, 2, 5, 6, 4, 7, 16)
boxplot(dt6)
q4
...
```{r}
plot(lake$DO, lake$riq)
cor(lake$DO, lake$riq)
cor.test(lake$DO, lake$riq)

plot(lake$DO, lake$WD)
cor(lake$DO, lake$WD)
cor.test(lake$DO, lake$WD)

cor.test(lake$DO, lake$WD, method = "spearman")
cor.test(lake$DO, lake$WD, method = "kendall")
```
