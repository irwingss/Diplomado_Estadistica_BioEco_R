...
# Cálculo de la moda
calcular_moda(dt1)
calcular_moda(lake$Season)
calcular_moda(lake$Lake)

# Cálculo sin función moda
table(dt1) |> sort(decreasing = T)
table(lake$Season) |> sort(decreasing = T)
table(lake$Lake) |> sort(decreasing = T)
```
...
**Donde**:
- $X_i$ y $Y_i$: Valores de las variables $X$ e $Y$.
- $\bar{X}$ y $\bar{Y}$: Promedios de las variables $X$ e $Y$.
- $n$: Número de observaciones.
- $\text{Cov}(X, Y)$: Covarianza entre $X$ e $Y$.


**Interpretación**: 
Los valores de la covarianza no están normalizados y pueden variar en magnitud, lo que hace que su interpretación absoluta (más allá del signo) sea difícil (a magnitud de la covarianza directamente, ya que depende de las escalas de ambas variables. En nuestro caso sería algo como mg/L por especie. Raro ¿no?).

```{r}
cov(lake$DO, lake$riq)
```
...
- $\sigma_X$ y $\sigma_Y$: Son las desviaciones estándar de las variables $X$ e $Y$, respectivamente.
- $\rho_{X,Y}$: Es el coeficiente de correlación de Pearson entre las variables $X$ e $Y$.

**Interpretación**: 
El coeficiente de correlación de Pearson ($\rho$) toma valores entre -1 y 1, lo que facilita su interpretación:
...
```{r}
plot(lake$DO, lake$SD)
plot(lake$DO, lake$TP)
cor(lake$DO, lake$TP)

boxplot(lake$TP)

atipicos <- boxplot.stats(lake$TP)$out
index <- which(lake$TP %in% atipicos)
boxplot(lake$TP[-index])

plot(lake$DO[-index], lake$TP[-index])
cor(lake$DO[-index], lake$TP[-index])
```
